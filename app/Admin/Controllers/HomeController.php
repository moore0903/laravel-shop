...
class HomeController extends Controller
{
    public function index()
    {
        return Admin::content(function (Content $content) {
            ...
            $shop_item_count = ShopItem::count();

            $content->row(function ($row) use($user_count,$order_count,$article_count,$shop_item_count) {
                $row->column(3, new InfoBox('会员数', 'users', 'aqua', '/admin/user', $user_count));
                $row->column(3, new InfoBox('订单数', 'shopping-cart', 'green', '/admin/order', $order_count));
                $row->column(3, new InfoBox('文章数', 'book', 'yellow', '/admin/article', $article_count));
                $row->column(3, new InfoBox('产品数', 'file', 'red', '/admin/shopitem', $shop_item_count));
            });
    ...
    public function imageUpload(Request $request){
        $file = Input::file('wangEditorH5File');
        if ($file->isValid()) {
